...
img_size = 224
species = 'species'
train_folder = './data_species/train-balance3'
valid_folder = './data_species/valid-species'
...
def train_model(model: nn.Module, maxepoch: int, save_name: str=None):
    ...
    if save_name:
        torch.save(best_model_wts.state_dict(), f'./models/{species}_shufflenetv2_{save_name}_params.pkl')
        logger.info(f'./models/{species}_shufflenetv2{save_name}_params.pkl')
        
model = ShuffleNetV2(scale=0.5, in_channels=9, c_tag=0.5, num_classes=10, activation=nn.ReLU, SE=False, residual=False)

train_model(model, 30, t)
